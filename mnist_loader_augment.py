...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(28*28, 128) #28*28 = 784 inputs, 128 neurons
        self.act = nn.ReLU()
        self.fc2 = nn.Linear(128, 10) #128 neurons, 10 outputs
...
def main():
    ...
    out_dir = os.path.join("outputs", f"mnist__augment__run_{run_id}")
    ...
    writer.add_text(
        ...
        "\n".join(
            [
                ...
                "",
                "Den här körningen tränar samma FFN-baseline som tidigare, men med data augmentation på träningssetet. Tanken är att se om augmentation hjälper en enkel modell eller om den bara blir förvirrad.",
                "",
                "### Det jag faktiskt följer",
                "- **01_Förlust (Loss)**: lägre = bättre. Hur 'fel' modellen är.",
                "- **02_Träffsäkerhet (Accuracy)**: högre = bättre. Andel rätt.",
                "- **Train vs Val**:",
                "  - **Train** = data modellen tränas på (här *med* augmentation).",
                "  - **Val** = data den *inte* uppdaterar vikter på, utan augmentation.",
                "",
