    ...
    writer.add_text(
        ...
        "\n".join(
            [
                ...
                "",
                "Den här körningen är 3-layer CNN igen, men nu med regularization påslagen. Det jag vill svara på är om allt det här tillsammans faktiskt drar upp val/test-resultatet jämfört med samma modell utan regularization.",
                "",
                "### Regularization-knappar jag har vridit på",
                "- **Batch normalization** efter conv-lager och första FC-lagret",
                "- **Dropout** efter första FC-lagret",
                "- **Weight decay** i Adam-optimizern",
                "- **Gaussian noise injection** i train augmentation",
                "",
                "### Vad jag förväntar mig",
                "- Train-resultatet kan bli lite sämre eftersom modellen får jobba mot brus och dropout.",
                "- Val/test bör bli bättre eller åtminstone stabilare — det är där jag faktiskt vill se vinsten.",
            ]
        ),
        0,
    )
    ...
    writer.add_text(
        ...
        "\n".join(
            [
                ...
                f"- Gaussian noise med std={training_config['gaussian_noise_std']:.2f}",
                "",
                "Regularization som ligger inne i själva modellen:",
                f"- Dropout p={training_config['dropout_p']:.2f}",
                f"- Weight decay {training_config['weight_decay']:.1e}",
                "- BatchNorm efter conv-lagren och fc1",
                "",
                "Val/test rör jag inte — bara ToTensor + Normalize, så jämförelsen blir rättvis.",
            ]
        ),
        0,
    )
